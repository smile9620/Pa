...
public final class EncodingHandler {
	 ...
	 * MultiFormatWriter().encode(str,BarcodeFormat.QR_CODE, 400, 400); int
	 * width = matrix.getWidth(); int height = matrix.getHeight();
	 * //二维矩阵转为一维像素数组,也就是一直横着排了 int[] pixels = new int[width * height]; for (int
	 * y = 0; y < height; y++) { for (int x = 0; x < width; x++) {
